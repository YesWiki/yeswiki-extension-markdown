...
if ($this->userIsAdmin()) {
    ...
    foreach($pages as $page) {
        ...
        $body = preg_replace('/==(.*?)==/uim', '##### $1', $body);
        // liens
        $body = preg_replace('/\[\[(.*?)( )(.*?)\]\]/uim', '[$3]($1)', $body);
        // italique
        $body = preg_replace('/\/\/(.*?)\/\//uim', '*$1*', $body);
        // code
        ...
        echo '<h2>'.$page['tag'].'</h2>'.nl2br($body).'<hr>';
    ...
    echo $this->header()
     .'<div class="alert alert-success">'._t('handler wakka2markdown : toutes les pages ont été transformées en syntaxe markdown.').'</div>'
     .$this->footer();
} else {
    echo $this->header()
     .'<div class="alert alert-danger">'._t('handler wakka2markdown : réservé aux administrateurs.').'</div>'
     .$this->footer();
}